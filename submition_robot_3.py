...
def make_history(command):
    """
    Takes command as arg and appends it to history in the variable log
    """
    global replay_count
...
def replay(robot_name, n, m):
    """
    replays in 3 ways
    1. replays all history in log without limitting
    2. replays last few commands where arg n sets the number
    3. replays commands in range between n and m
    """
...
def show_replayed(robot_name):
    """
    prints out number of replayed commands reversed or
    silently depending on the state of the program
    """
    if on_replay_range:
...
def replay_silent(robot_name,n):
    """
    silently replays history of commands in 2 ways
    1. replays last n of the history
    2. replays all the history
    """
...
def replay_reversed(robot_name, n):
    """
    replays history of commands in reverse in 2 ways
    1. replays last n of the history
    2. replays all the history
    """
    global current_direction_index, replay_count
...
def valid_range(int_lst):
    """
    makes sure the range(eg 4-2) provided in int_lst
    is valid by returning true if first digit is biger
    than the second,
    prints out a message to the user to fix it if not
    """
...
def replay_reversed_silent(robot_name):
    """
    replays history of commands in silent and in reverse
    """
    global current_direction_index
    current_direction_index = 0
    i = 0
...
def rearange_command(command):
    """
    takes command and rearanges the order of it
    returns it in a form understood by the program
    eg 'replay 2 silent' becomes 'replay silent 2' 
    """
    x = command.split()
    new_x = []
    new_x.append(x[0])
